...
- [ ] **T5-24 긴 클립 학습.** 학습은 8프레임, 배포는 수백 프레임이다. 모델은 30프레임 동안
  게이팅을 견디는 법을 배운 적이 없다(§4.30d). clip_len 8 → 24~32로 올려 재학습.
  **가장 싸고 가장 직접적인 수정**이며 다른 모든 항목과 독립이다.
- [ ] **T5-25 키프레임 주기 재조정.** 톱니가 해법이 이미 있음을 보여준다 — 너무 드물게 쓸 뿐.
  주기 30 → 10/15에서 정확도-연산 곡선을 그린다. 연산은 늘지만 §4.27b의 2.2배 예산이 있다.
...
- [ ] **T5-32 계통 오차 점검.** 2 DOF 정렬이 실촬 AbsRel을 0.1595 → 0.1321로 **17% 개선**한다
