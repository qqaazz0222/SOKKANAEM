...
| W5 | ~~Jetson·전력 실측 없음~~ **해소** — Nano B01 지연·에너지 실측(§4.37). TX2·Pi 4B 미측정 | T2-11 |
...
- [x] **T2-11 · W5 엣지 실측 — Jetson Nano B01에서 완료(2026-08-21).** 이 측정이 무엇을
  ...
  **남은 공백 셋**: `sm_53`이 융합 커널을 못 돌려 측정 구성이 배포 구성이 아니고(Orin급 필요),
  이 기기는 실시간과 멀며(8.8 FPS), **TX2와 Raspberry Pi 4B는 아직 미측정**이다
  (절차만 `docs/EDGE_BENCH.md`에 있다).
