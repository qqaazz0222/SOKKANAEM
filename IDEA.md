...
(49.6 MB)로 **2.2배 앞선다**. ~~정확도도 실촬 0.1595 vs 0.2256으로 우위다.~~ — **정확도
우위 주장은 철회한다**(2026-08-23): 0.2256은 정합 아티팩트였다(논문 §5.3).
다만 이 값은 **dense 경로**의 것이며, 희소 경로는 4090에서 항상 더 느리다(§4.24d).
**Jetson Nano B01 실측으로 그 서열이 기기 종속임이 확인됐다**(§4.37): 연산에 묶인 기기에서는
5% active에서 희소가 13.7배 빠르고 에너지 15.3배 적다. 성립 조건은 "제거하는 연산이 제거를
결정하는 비용을 넘을 때"이며, 4090에서는 그 조건이 성립하지 않는다.
