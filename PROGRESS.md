...
1. **논문 완성** — 남은 유일한 작업. (a) 참고문헌 작성(`draft.md` 말미가 아직 플레이스홀더),
   (b) §6 ablation 절이 v3/v4 기준이고 §6.3에 "results are pending"이 남아 있음,
   (c) `paper/draft_ko.md`가 2026-07-27자로 방치돼 영문판과 크게 다름
2. **T2-11 Jetson — 기기 미확보로 보류.** 계획 자체는 닫았다(PLAN `[-]`). 기기가 생기면
   `scripts/bench.py`를 그대로 돌리면 되고, 그때까지 희소성의 이득은 연산량·메모리로만 주장한다
