...
| 3. 시스템 (4주) | 🟡 커널 완료, 에지 실측만 남음 | 융합 Triton 스캔으로 dense compiled **1.29 ms(776 FPS)**, 희소 2.04 ms — 합격선 2.34 ms 통과. **단 4090에서는 dense가 항상 더 빠르다**(§4.24): 희소의 이득은 연산량 37.0%와 스트림당 state로 한정, 환산 여부는 Jetson 실측이 가른다 |
...
1. **T2-11 Jetson Orin 실측** — 남은 유일한 실험. 연산량 37.0% 절감이 시간·전력으로
   환산되는지 가른다. 4090에서는 오버헤드에 묶여 나타나지 않는다(§4.24d)
2. 논문 완성 — 참고문헌 정리, W1/W2 프레이밍 반영, §6 ablation 절 최신화
3. 실내 미지 도메인(NYU·ScanNet) zero-shot — 호스트 복구 또는 서명 절차 필요
4. GMC 임계 자동 보정 — 도메인별 수동 재조정이 현재 전제(§4.26c)
5. `tartanair_v2/Hospital` 간헐적 read 실패 — 재발하면 `num_workers` 조정
6. `baselines` env `transformers` 결손 — 재현성 정리 필요
   남는 한 이 이상은 안 나온다
