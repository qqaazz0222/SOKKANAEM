...
항등 fallback은 **0회** 발동했다. 이 분기가 대비하는 실패 양상(질감 부족, 퇴화 적합)은 여기서 이동
카메라 경로를 제한하는 요인이 아니다. 제한하는 것은 임계값의 스케일이다.
...
**표 7a. 클립 길이 대 정확도, 실촬 실내 holdout, 데이터셋 균형 평균. holdout이 유한하므로 클립 수는
길이와 함께 줄어든다 — 8프레임 189클립, 256프레임 13클립.**
...
보고하며 그것이 효과의 크기라고 다뤘다. 256프레임까지 재면 보고 체크포인트에서 87%다. 배치는 수백
프레임을 돌리므로, 이 구조가 대상으로 하는 설정을 서술하는 수치는 이쪽이다.
