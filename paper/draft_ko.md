...
5. **게이팅의 대가는 스트림을 따라 누적된다.** 유지된 상태는 프레임이 갈수록 정확도를 개선하지
   않고, 동적 장면에서는 키프레임이 초기화하기 전까지 61% 악화한다. 클립 단위 평균이 이를
   가리며, 표준 8프레임 프로토콜은 그 주기의 유리한 부분만 표본한다(§5.7).
...
### 5.7 스트리밍 드리프트 — 클립 단위 수치가 가리는 것 `[검증 중]`

> 이 절의 인과 주장 — 드리프트가 \(\Delta\)-게이팅 자체가 아니라 학습–배포 불일치에서 온다는 것 —
> 을 검증하려고 클립 길이 24의 fine-tune이 실행 중이다. 성공하면 표 7·8과 권장 갱신 주기가 모두
> 바뀌고, 실패하면 불일치 설명이 틀린 것이며 드리프트는 내재적이다.
...
8. 정확도가 키프레임 사이에서 악화하고, 보고한 클립 단위 수치는 그 주기의 유리한 부분을 표본하는
   ...
   270프레임 너머는 미측정이며, 평평하기보다 더 나빠질 것으로 본다.
