...
정확도는 최하위권이고 원시 프레임 간 차이는 8프레임 1.10배·256프레임 1.23배로 1위다 — 명시적 시간 모듈을 가진 영상 전용
...
**보고 모델과 그것을 만든 두 단계.** SOKKANAEM 모델은 모든 설정에서 4.19M 파라미터다. 학습은 세
단계로 보고 체크포인트에 도달하며, 그중 두 단계가 이 논문의 측정에 대응해 추가됐으므로 셋 다 이름을
붙인다.
...
주장이며, 이전 표에서 빠져 있던 baseline 부류를 넣어도 뒤집히지 않았다.
...
| **희소성에 의한** 실측 속도 향상 | **연산 제약 기기에서 확립, 데스크톱 GPU에서는 아님** | Jetson Nano 활성 5%에서 11.5배; 4090에서는 모든 활성률에서 컴파일 dense가 더 빠름 |
| 희소성에 의한 전력·에너지 절감 | **미측정** | 레일 파서가 이 기기의 `tegrastats` 형식을 못 잡았다. 수정 완료, 재실행 한 번 남음 |
| 엣지 가속기에서 **보고 구현**의 우위 | **미측정** | 보유 기기가 Volta 이전 SM이라 Triton이 동작하지 않아 엣지 수치는 참조 스캔이다 |
...
**표 13. Jetson Nano B01에서 강제 활성률 대비 프레임당 지연, 256픽셀, fp32, 참조 스캔. dense는 두
캐시를 모두 끈 같은 모델이다.**

| 활성률 (%) | 희소 (ms) | dense (ms) | 배수 | 5W에서 희소 (ms) |
|---:|---:|---:|---:|---:|
| 5.1 | **139.14** | 1592.49 | **11.5배** | 199.64 |
| 14.8 | 279.41 | 1590.07 | 5.7배 | 396.61 |
| 30.1 | 539.74 | 1590.80 | 3.0배 | 728.79 |
| 50.0 | 842.16 | 1588.39 | 1.9배 | 1134.05 |
| 69.9 | 1161.73 | 1586.96 | 1.4배 | 1569.75 |
| 100.0 | 1663.05 | 1586.92 | 0.95배 | 2189.90 |

세 가지가 따라오며, 합치면 §5.8의 추측이 측정으로 바뀐다.

**기기가 연산 제약이면 희소성은 시간으로 환산된다.** dense 지연은 4090과 똑같이 활성률에 평평하다
(스윕 전체에서 1587~1592 ms). 반면 희소 경로는 활성률에 따라 움직여 전체 갱신 1663.05 ms에서 활성
5%의 139.14 ms까지 내려간다. 최소제곱 적합은 `희소 ≈ 49 ms + 1605 ms × 활성률`이다 — 활성률에 비례하는
항이 연산이고, 49 ms 절편이 부기 비용이다. 4090에서는 그 절편이 전부였다.

**교차점이 주장이 아니라 수치로 나온다.** 이 기기에서 희소가 dense에 지는 것은 활성률 96% 이상,
즉 부기 비용이 상쇄되지 않는 구간뿐이다. 두 기기를 함께 읽으면 — 메커니즘은 제거하는 산술이 무엇을
제거할지 정하는 고정 비용을 넘을 때 이득이고, 그 조건이 10W SoC에서는 11배로 성립하고 융합 커널을
가진 데스크톱 카드에서는 아예 성립하지 않는다.

**저전력 모드는 규모를 바꾸고 형태는 바꾸지 않는다.** 5W(CPU 코어 4개 대신 2개, 낮은 클럭)에서 모든
수치가 약 1.3배 커지지만 배수는 유효숫자 두 자리까지 같다 — 활성 5%에서 10.9배, 손익분기 99%.
교환 곡선은 전력 예산이 아니라 모델과 마스크의 성질이다.

확립되지 않는 것: 이 기기는 실시간과 거리가 멀다(활성 5%에서 7.2 FPS, dense 0.6 FPS). 즉 산술이
시간으로 환산된다는 서술이고 배치 가능한 처리량 주장이 아니다. 그리고 `sm_53`이 융합 커널을 못 돌리므로
여기서 측정한 구성은 우리가 배포하는 구성이 아니다. Orin급 기기면 두 공백이 함께 닫힌다.
...
무겁게 보는 배치라면 장클립 체크포인트를 주기 60으로 돌리는 편이 맞다. 어느 선택도 감추지 않도록
표 3에 두 행 모두 있다.
...
5. 이 규모에서 RTX 4090 실행은 오버헤드 제약이라 해석적 MAC 절감이 *거기서는* FPS로 이어지지
   않는다. 연산 제약인 Jetson Nano에서는 이어진다 — 활성 5%에서 11.5배(§5.7). 두 공백이 남는다.
   그 엣지 기기는 우리가 배포하는 융합 커널을 못 돌리므로(Triton이 `sm_53` 미지원) 역전된 순서는
   참조 스캔에서 측정됐고, 프레임당 에너지는 레일 파서가 이 기기의 `tegrastats` 형식을 놓쳐 여전히
   미측정이다. Orin급 기기면 둘 다 닫힌다.
...
12. base 체크포인트는 60k 단일 seed다. 최종 단계는 seed 3개로 반복 중이며, 그것이 끝나기 전까지 8k
    노이즈 바닥보다 작은 차이는 이 논문 어디에서도 주장하지 않는다.
