...
**가장 날카로운 결과는 프로토콜에 관한 부정적 결과다.** 클립 길이가 답을 바꾼다 — 오차가 8프레임에서
512프레임 클립으로 갈 때 128% 커지고, 긴 클립 fine-tuning은 그중 5분의 1을 없애면서 8프레임 수치는
소수점 네 자리까지 그대로 둔다. 즉 이 분야가 쓰는 8프레임 관행은 스트리밍 거동을 가장 크게 개선하는
개입에 눈이 멀어 있다. 희소성의 다른 주장들도 비슷하게 경계가 있다 — 융합 스캔 커널 이후 데스크톱
