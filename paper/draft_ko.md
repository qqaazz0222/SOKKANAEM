...
**그림 1. 스트리밍 파이프라인.** 변화 검출기가 패치 활성 마스크를 만들고, 그 마스크가 백본에
\(\Delta\)-게이팅 신호로 전달된다. 공간 출력과 시간 hidden state, 두 캐시가 MAC 절감의 실제
출처다. 전역 모션 보정 분기는 이동 카메라에서만 쓴다. 활성 40%를 넘는 프레임은 dense 경로를 탄다.
...
**그림 2. \(\Delta\)-게이팅.** 마스크가 이산화 스텝에 곱해지므로 정적 패치는 항등 전이를 받고 입력
항이 사라진다. 스킵을 보정하는 것이 아니라, 스킵이 상태 보존과 대수적으로 동일하다.
...
**그림 3. 학습에 쓰지 않은 실촬 실내 영상의 정성 결과** (RGB, 예측, 정답. 이 프레임의 활성 패치는
3.5%). 예측이 정답보다 매끄러운데, 이는 §6.4가 정량화하는 바와 같다 — 모델이 자신의 패치 그리드
상한보다 한참 위에 있으므로, 세부 손실은 출력 해상도가 아니라 용량·학습의 한계다.
