...
4.19M 파라미터 모델은 실촬 실내 holdout에서 프레임당 패치 22.0%를 갱신하며 AbsRel 0.1302에 도달하고,
갱신율을 4.6%로 낮추는 대가는 상대 오차 6.4%다. 하나의 프로토콜로 측정한 상용 깊이 모델 7종 대비
정확도는 최하위권이고 원시 프레임 간 차이는 1.27~1.36배로 1위다 — 명시적 시간 모듈을 가진 영상 전용
baseline에 대해서도 그렇다. 모션 보정 지표와 GT 기준 지표에서는 앞서지 않으며, 일반적인 시간 일관성
우위를 주장하지 않는다.
...
fine-tune 모델의 프레임별 오차는 첫 프레임에서 천 번째 프레임까지 1.1% 오르는데 클립 단위 점수는 36%
악화한다. 즉 드리프트처럼 보이는 것의 대부분은 클립당 정합 창이고, 상태 없는 baseline이 우리보다 더
크게 그 영향을 받는다. 희소성의 다른 주장들도 비슷하게 경계가 있다 — 융합 스캔 커널 이후 데스크톱
GPU에서는 모든 활성률에서 dense 실행이 희소 경로보다 빠르므로, 희소성의 이득은 측정된 시간이 아니라
산술과 스트림당 상태에서 성립한다.
...
**표 7d. 1,024프레임 스트림. 클립 단위 점수는 클립당 배율 하나를 쓰고, 프레임별 열은 각 프레임을
독립 정합하므로 정합 창 없이 드리프트만 본다.**

| 소스 | 클립 수 | 체크포인트 | 클립 AbsRel | 8프레임 대비 | 프레임별 AbsRel, 프레임 0 → 1023 | 드리프트 |
|---|---:|---|---:|---:|---|---:|
| PointOdyssey | 20 | 보고 | 0.5344 | +58% | 0.2842 → 0.3227 | +13.5% |
| PointOdyssey | 20 | 장클립 | **0.4638** | **+36%** | 0.2700 → **0.2730** | **+1.1%** |
| Bonn (static\_close\_far) | 1 | 보고 | 0.4820 | — | — | — |
| Bonn (static\_close\_far) | 1 | 장클립 | **0.4170** | — | — | — |

**클립 단위 벌점의 거의 전부가 프로토콜이고, 남은 부분의 거의 전부는 긴 클립 학습이 없앤다.** 장클립
체크포인트의 1,024프레임 클립 단위 점수는 8프레임보다 36% 나쁘지만, 같은 모델의 독립 정합 프레임별
오차는 첫 프레임에서 천 번째 프레임까지 1.1% 오르고 \(\delta_1\)은 1.4포인트 떨어진다. 천 프레임 동안
실제로 드리프트한 모델은 평평한 프레임별 곡선을 낼 수 없다. 보고 체크포인트는 드리프트한다 — 같은
구간에서 13.5%, \(\delta_1\) 8.5포인트 — 즉 드리프트는 실재하고 긴 클립 학습이 그 대부분을 제거한다.

이것이 8프레임 관행으로는 물을 수조차 없는 질문에 대한 답이다. **천 프레임에 걸친 상태 보존은
안정적이고, 천 프레임에 걸친 클립당 정합은 안정적이지 않다.** 프레임별로 정합하거나 metric 배율을
가진 배치는 짧은 클립 표가 광고하는 정확도를 유지한다. 긴 클립에 배율 하나를 맞추는 벤치마크는 그
안의 어떤 모델보다 자기 프로토콜에 대해 더 많이 말하는 악화를 보고하게 된다 — 상태 없는 baseline이
그중 가장 크게 악화한다는 점까지 포함해서(표 3a).
...
7. 교차 도메인 평가는 실촬 주행만 다룬다. 미지의 실내 도메인(NYU, ScanNet)은 평가하지 않았다 —
   ...
   종류의 합성→실촬 축에 한정된다.
8. 클립당 정합에서 클립 길이 의존성이 심하다 — 보고 체크포인트는 8프레임에서 AbsRel 0.1302,
   실촬 512프레임에서 0.2972다(§5.8). 1,024프레임 측정이 원인을 분리한다 — 프레임별 드리프트는
   보고 체크포인트 13.5%, 장클립 1.1%인데 클립 단위 벌점은 58%와 36%다. 즉 효과의 대부분은 정합
   창이다. 우리가 할 수 없는 것은 비교 표에서 정합 창을 없애는 것이다 — 배율이 모호한 깊이는 어떻게든
