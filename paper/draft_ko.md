...
**세 가지 결과가 기여의 경계를 긋는다. 각각은 처음에 하려던 주장을 하나씩 좁힌다.** 지배적
...
오르며, 모션 보정과 도메인별 임계 재조정을 거쳐야 14%로 되돌아온다. 그리고 희소 경로를 학습하지
않았던 이전 체크포인트에서 4배 붕괴했던 동일 마스크 토큰 제거 대조군이, 지금은 정확도상 동률이다
— **보존된 상태의 readout은 정확도가 아니라 시간 안정성을 산다.** 이 결과들은 정확한 변화 게이팅
...
2. Video Depth Anything이 baseline 표에 없다. 로컬 체크아웃이 소실됐고 metric 가중치 재확보는 이번
   라운드 범위 밖이었다. 따라서 비교는 동급 규모 1종과 훨씬 큰 1종을 다룬다.
...
10. 60k 결과는 단일 seed다. seed 분산은 8k에서만 특성화했으며(부록 A), 그 노이즈 바닥 아래의 차이는
    어디에서도 주장하지 않는다.
