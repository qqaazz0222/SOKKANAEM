...
**클립 단위 벌점의 대부분은 프레임별 정합에서 사라지고, 남은 부분의 거의 전부는 긴 클립 학습이
없앤다.** 장클립 체크포인트의 1,024프레임 클립 단위 점수는 8프레임보다 36% 나쁘지만, 같은 모델의 독립
정합 프레임별 오차는 첫 프레임에서 천 번째 프레임까지 1.1% 오르고 \(\delta_1\)은 1.4포인트 떨어진다.
천 프레임 동안 국소 깊이 *형상*이 실제로 악화한 모델은 평평한 프레임별 곡선을 낼 수 없다. 보고
체크포인트는 악화한다 — 같은 구간에서 13.5%, \(\delta_1\) 8.5포인트 — 즉 그 실패 양상은 실재하고 긴
클립 학습이 그 대부분을 제거한다.

**두 열 사이의 격차가 무엇으로 이루어져 있으며, 왜 단순한 벤치마크 인공물이 아닌가.** 프레임별 정합은
정의상 프레임별 전역 배율 형태의 오차를 제거한다. 따라서 36% 대 1.1%라는 격차는 긴 클립이 드러내는
것이 국소 깊이 형상의 드리프트가 아니라 시간에 따라 변하는 전역 배율이라는 뜻이다 — 그리고 흔들리는
전역 배율은 프로토콜의 성질이 아니라 모델의 성질이다. metric 배율을 가졌거나 프레임별로 정합하는
배치에는 보이지 않지만, 긴 스트림에 배율 하나를 들고 가는 배치는 정확히 그 값을 지불한다. 보고 모델의
프레임별 인자 \(s_t\)에서 직접 재면, 그 크기는 구간이 길어질수록 커진다:
