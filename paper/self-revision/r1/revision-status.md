...
> 2026-09-07 정정: 아래는 당시 초안의 대응 기록이다. 현재 제출본의 해결 여부는
> [R3 대응표](../r3/revision-status.md)를 따른다. “전건 해결”은 현재 원고/증거를 재검증한 판정이 아니다.
...
- **두 번째 기기(2026-08-24)**: Raspberry Pi 4B(CPU 전용, GPU 경로 없음)에서 활성 5.1%에
  **14.2배**. `희소 ≈ 169 ms + 6439 ms × 활성`(R²=0.9989)이고 고정 부기가 dense 프레임의 2.4% —
  Nano의 2.1%와 사실상 같다. 손익분기 105%로 **이 기기에서는 희소가 지지 않는다**.
  Nano에 GPU가 있으므로 "어느 가속기냐의 문제"라는 반론이 가능했는데, 가속기 없는 보드가 그것을
  닫는다. → 논문 §5.7 표 13b, `measures/pi-4b.log`
- **남은 공백**: 두 기기 모두 융합 커널을 못 돌린다(`sm_53`도 CPU도 Triton 미지원)이라 측정 구성이
  배포 구성이 아니다 — Orin급 필요. Pi는 읽을 수 있는 전력 레일이 없어 **에너지는 Nano 하나에
  기댄다**. **Jetson TX2는 미측정**이며 절차는 `EDGE_BENCH.md`에 있다.
