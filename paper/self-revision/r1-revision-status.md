...
**요약(2026-08-23 갱신)**: major 9건 **전건 해결**. minor 15건 중 13건 해결·2건은 최종
교정본으로 이월(minor 1 태그 제거, minor 15 저자·소속 — 사용자 정보 필요).

> 이 문서는 2026-08-21 01:20에 작성됐고 그 뒤 두 항목이 닫혔다. major 5(엣지)는 같은 날
> 16:26 Jetson Nano B01 실측으로, major 8(multi-seed)은 seed 3개 완료로 닫혔다.
> minor 12(정성 예시)는 08-23 Figure 9 생성으로 닫혔다.
...
### 5. efficiency 주장 분리 — 해결(2026-08-21)
...
- **해결(2026-08-21)**: Jetson Nano B01을 확보해 지연·전력·에너지를 전부 측정했다. 5% active
  에서 시간 13.7배·에너지 15.3배, 손익분기 active 99%. 4축 채점표에서 "희소성에 의한 속도
  향상"과 "에너지"가 미확립에서 **연산에 묶인 기기에서 확립**으로 바뀌었다. → REPORT §4.37
- **남은 공백**: `sm_53`이 융합 커널을 못 돌려 측정 구성이 배포 구성이 아니다(Orin급 필요).
  **TX2·Raspberry Pi 4B는 미측정**이며 절차만 `docs/EDGE_BENCH.md`에 있다.
...
### 8. 통계 신뢰도(multi-seed) — 해결(2026-08-21)

최종 단계(clip 24, 8k, spread 0.5)를 seed 0/1/2로 반복 완료. 프로토콜별 mean±std가 부록 A에
붙었고 8프레임 AbsRel 표준편차는 **±0.0012**로, 그동안 인용하던 8k 프로브 추정(±0.005)보다
4~8배 좁다. **spread 항의 3~4% 정확도 이득은 CI 안이라 주장하지 않는다** — 범위비 개선만 남았다.
이 분산이 있어야 물을 수 있던 질문(warp 가중치 스윕)이 그 뒤에 이어졌다. → REPORT §4.40

범위 한정: 세 run은 base·장클립 초기화를 공유하고 마지막 8k의 seed만 다르므로 그 단계의 분산이지
파이프라인 전체의 분산이 아니다. base는 여전히 60k 단일 seed다.
...
| 12 | 정성 예시 | 해결 | 키프레임 전후 패널(Figure 8) + **정성 비교(Figure 9, 2026-08-23)**. 폴백이 마스크를 전부 1로 덮어쓰는 문제는 검출기 마스크를 따로 뽑아 해결 — 마스크 열은 검출기의 선택, 깊이 열은 배포 설정 |
