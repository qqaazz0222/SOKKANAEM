...
| Wall-clock speedup *from sparsity* | **demonstrated where arithmetic is the constraint, not on a desktop GPU** | 13.7x at 5% activity on a Jetson Nano; compiled dense is faster at every activity level on a 4090 |
| Energy per frame reduction from sparsity | **demonstrated on the same device** | 15.3x at 5% activity, from finishing sooner rather than drawing less (Table 13) |
...
**Table 13. Per-frame latency and energy against forced activity on a Jetson Nano B01, 256 pixels, fp32, reference scan, 10W mode. Dense is the same model with both caches disabled. Power is the board input rail read from the INA3221.**
...
5. Execution on an RTX 4090 is overhead-bound at this model scale, so reduced analytical MACs do not become higher FPS *there*; on a compute-bound Jetson Nano they do — 13.7x in time and 15.3x in energy per frame at 5% activity (Section 5.7). One gap remains: that device cannot run the fused kernel we ship (Triton does not target `sm_53`), so the inverted ordering and the energy figures are measured on the reference scan. An Orin-class device would close it.
