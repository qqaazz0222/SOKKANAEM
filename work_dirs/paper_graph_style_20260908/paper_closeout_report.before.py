...
def main():
    ...
    figure = fitz.open()
    page = figure.new_page(width=620, height=450)
    for index, r in enumerate(diagnostics):
        fs = r['frames']
        left, top, width, height = 40+310*(index % 2), 35+220*(index//2), 250, 145
        ...
        page.insert_text((left, top-17), title, fontsize=8)
        page.insert_text((left, top-5), 'Internal state RMS', fontsize=7)
        page.draw_rect(fitz.Rect(left, top, left+width, top+height), color=(.5, .5, .5), width=.4)
        for tick in (0, 60, 120, 180, 255):
            page.insert_text((left+tick/255*width-4, top+height+11), str(tick), fontsize=6)
        for fraction in (0, .5, 1):
            page.insert_text((left-25, top+height*(1-fraction)+2), f'{high*fraction:.2f}', fontsize=6)
        for f in fs:
            if f['keyframe']:
                x = left+f['frame']/255*width
                page.draw_line((x, top), (x, top+height), color=(.7, .7, .7), width=.4, dashes='[1 2]')
        for field, color in [('net_state_error_rms', (.086, .357, .588)),
                             ('net_local_defect_rms', (.749, .341, .231))]:
            points = [(left+f['frame']/255*width, top+height*(1-f[field]/high)) for f in fs]
            page.draw_polyline(points, color=color, width=.7)
        page.insert_text((left+65, top+height+22), 'Frame within development clip', fontsize=7)
    page.insert_text((40, 442), 'Blue: dense-history difference    Red: same-state local defect    Dotted: keyframe', fontsize=8)
