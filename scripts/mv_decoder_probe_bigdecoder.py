...
class AllTapsBackbone(HighResDistilledBackbone):
    """HighResDistilledBackbone, but returns every stage-2 block's tokens (8), not the
    TAPS=(1,3,5,7) subset scripts/mv_decoder_probe.py hardcodes."""
...
def train(args):
    ...
    decoder = build_wide_decoder(backbone.dim, N_TAPS).to(dev).train()
    bin_logits = []
...
def score(args):
    ...
    print("\n8-taps F-equivalent, combined metrics:", combined)
