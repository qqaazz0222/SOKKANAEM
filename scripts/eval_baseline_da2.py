"""Depth Anything V2 baseline on the exact same holdout clips/protocol as
...
def main():
    ...
    model = AutoModelForDepthEstimation.from_pretrained(CKPT).to(dev).eval()
    ...
    with torch.no_grad():
        for ci, (clip, gt, valid) in enumerate(loader):
            ...
            for t in range(T):
                ...
                out = model(**inputs)
                post = proc.post_process_depth_estimation(out, target_sizes=[clip.shape[-2:]])
            ...
            # least-squares scale+shift: y ~= s*x + b (standard scale-shift-
            # invariant protocol for relative-depth models)
            A = np.stack([x, np.ones_like(x)], axis=1)
            (s, b), *_ = np.linalg.lstsq(A, y, rcond=None)
            aligned_disp = s * pred.numpy() + b
            depth_pred = torch.from_numpy(1.0 / np.clip(aligned_disp, 1e-3, None)).float()
    ...
    report(f"DA-v2-Small (24.8M){tag}", acc)
