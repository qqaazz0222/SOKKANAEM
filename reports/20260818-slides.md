...
<div class="badge">스트리밍 776 FPS</div>
...
<div class="cap">그림 4. 융합 스캔 커널 전후의 프레임당 지연 시간 — 모든 경로가 빨라졌고, 가장 빠른 것은 희소가 아니라 전체 연산이 되었다.</div>
...
<div class="cap">표 7. 변화 패치 22%에서의 프레임당 지연 시간 (단일 GPU, 256px, 단일 스트림). 커널 적용 후에는 모든 경로가 변화 패치 비율에 거의 무관하게 평평하다.</div>
...
- **뒤집힌 결론** — 목표선 2.34 ms는 통과했지만 같은 커널이 희소 경로의 존재 이유도 없앴다. **이 GPU에서는 모든 변화 비율에서 전체 연산이 더 빠르다**
...
- **미지 도메인 실측** (실촬 주행 885프레임, 미학습)
  ...
  - **희소성은 안 넘어간다** — 변화 패치 25.8% → **92.8%**
  - 센서 노이즈·노출·롤링 셔터가 전부 변화로 잡힘. 고정 카메라 수치는 실측이지만 **합성 주행 스킵률은 낙관값**
...
- **여전히 미검증** — 에지 지연·전력, 실내 미지 도메인, 핸드헬드·항공 모션, 조건당 1회 실행
...
② 4M 파라미터로 스트리밍 776 FPS, 프레임 간 변동은 30배 큰 모델보다 작다<br/>
